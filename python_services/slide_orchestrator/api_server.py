...
import asyncio
# Lazy import heavy modules only inside endpoints that need them to keep
# serverless health/check endpoints lightweight on Vercel.
try:
    from .simple_flow import generate_and_store_plan, generate_two_slides_via_llm
except Exception:
    generate_and_store_plan = None  # type: ignore
    generate_two_slides_via_llm = None  # type: ignore
...
async def generate_slides(req: SlideRequest):
    try:
        from .graph import run_demo  # defer heavy import
    except Exception as e:
...
async def stream_slides_simple(req: SlideRequest):
    if generate_and_store_plan is None or generate_two_slides_via_llm is None:
        raise HTTPException(status_code=503, detail="orchestrator simple flow unavailable: dependencies not installed")
...
async def stream_slides(req: SlideRequest):
    try:
        ...
        raise HTTPException(status_code=503, detail=f"orchestrator graph unavailable: {e}")
    """
